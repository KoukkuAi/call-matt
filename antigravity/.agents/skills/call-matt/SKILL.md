...
| Skill | Use when |
|---|---|
| `/git-guardrails-claude-code` | NOT APPLICABLE here � it installs hooks specific to another agent (Claude Code) and won't work in Antigravity. If the user wants git safety, suggest an always-on Antigravity Rule in `.agents/rules/` forbidding destructive git commands instead. |
...
11. **Afraid the agent will wreck the git history** → don't route to `/git-guardrails-claude-code` (it's for another agent); instead offer to write an always-on Rule in `.agents/rules/` that forbids destructive git commands. **Wants commit-time checks** → `/setup-pre-commit`.
